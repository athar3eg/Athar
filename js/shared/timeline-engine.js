...
function buildDailyTimeline(opts) {
  ...
  // حساب كم جلسة ستنقسم إليها كل مهمة مسبقاً لعرضها بأسلوب ذكي وأنيق
  const taskTotalSessions = {};
  sortedTasks.forEach(t => {
    const mins = t.estimated_minutes || 30;
    taskTotalSessions[t.id || t.title] = Math.max(1, Math.ceil(mins / sessionMinutes));
  });
  const taskCurrentSession = {};
  ...
  for (const slot of freeSlots) {
    ...
    while (pos < slot.end && taskIdx < remainingTasks.length) {
      ...
      taskCurrentSession[taskId] = (taskCurrentSession[taskId] || 0) + 1;
      const currentPart = taskCurrentSession[taskId];
      const totalParts = taskTotalSessions[taskId] || 1;

      const partLabel = totalParts > 1 ? ` · (جلسة ${currentPart} من ${totalParts})` : "";
      const displayTitle = `${task.title}${partLabel}`;

      timeline.push({
        start: pos,
        end: pos + chunk,
        startLabel: formatTimelineMinutes(pos),
        type: "study",
        label: displayTitle,
    ...
    if (pos < slot.end) {
      const freeDur = slot.end - pos;
      // لا تعرض فترات حرة تافهة أقل من 15 دقيقة لتجنب حشو الجدول
      if (freeDur >= 15) {
        timeline.push({
          start: pos,
          end: slot.end,
          startLabel: formatTimelineMinutes(pos),
          type: "free",
          label: "وقت حر — مراجعة خفيفة أو راحة"
        });
      }
    }
  }
