...
(function () {
  class GeminiAgent {
    ...
    // ── قائمة كل الـ API Keys: localStorage أولاً + Supabase
    get apiKeys() {
      const raw = localStorage.getItem('athar_gemini_api_key') || localStorage.getItem('madar_gemini_api_key') || '';
      return raw.split(',').map(k => k.trim()).filter(k => k.length > 0);
    }

    // ── جلب الـ keys من Supabase (gemini_api_keys table) مدموجة مع localStorage
    async getAllApiKeys(userId) {
      const localKeys = this.apiKeys;
      try {
        const { data } = await this.sb
          .from('gemini_api_keys')
          .select('api_key, priority, is_exhausted')
          .eq('user_id', userId)
          .eq('is_exhausted', false)
          .order('priority', { ascending: true });
        const dbKeys = (data || []).map(k => k.api_key).filter(k => k && k.length > 0);
        // دمج: localStorage أولاً ثم Supabase (بدون تكرار)
        const merged = [...new Set([...localKeys, ...dbKeys])];
        return merged.length > 0 ? merged : localKeys;
      } catch (_) {
        return localKeys;
      }
    }
    ...
    // ── تحديث الـ badge في الـ header مباشرة
    _updateBadge(modelName) {
      const badge = document.getElementById('agentBadge');
      const statusText = document.getElementById('agentStatusText');
      if (badge) {
        badge.innerHTML = `<span class="w-2 h-2 rounded-full bg-secondary animate-pulse"></span><span>${modelName} ⚡</span>`;
      }
      if (statusText) {
        statusText.textContent = `تحكم كامل عبر ${modelName}`;
      }
    }
...
`;
    ...
    async sendMessage(userId, userMessage, chatHistory = [], abortSignal = null) {
      ...
      const apiKeys = await this.getAllApiKeys(userId);   // localStorage + Supabase
      ...
      // ─ اختيار النموذج ومحاولة كل الـ keys
      let currentModel = this.modelName;
      let activeKey = apiKeys[0];
      let endpoint = `https://generativelanguage.googleapis.com/v1beta/models/${currentModel}:generateContent?key=${activeKey}`;

      // ── دالة تجرب كل الـ keys واحدة واحدة
      const callWithKeyRotation = async (payload) => {
        for (let ki = 0; ki < apiKeys.length; ki++) {
          activeKey = apiKeys[ki];
          let tryEndpoint = `https://generativelanguage.googleapis.com/v1beta/models/${currentModel}:generateContent?key=${activeKey}`;

          let res = await fetch(tryEndpoint, {
            method: "POST",
          ...
          if (res.ok) {
            ...
            // تحديث الـ badge بالموديل الحالي
            localStorage.setItem('athar_gemini_model', currentModel);
            this._updateBadge(currentModel);
            return res.json();
          }
          ...
          const errCode = errJson.error?.code || res.status;

          // لو مشكلة في الموديل (مش في الـ key) → جرب موديل تاني بنفس الـ key
          if (errCode !== 429 && errCode !== 401 && errCode !== 403) {
            ...
            if (suggestedMatch && suggestedMatch[1]) {
              ...
              try {
                const listRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${activeKey}`, { signal: abortSignal });
            ...
            localStorage.setItem('athar_gemini_model', currentModel);
            tryEndpoint = `https://generativelanguage.googleapis.com/v1beta/models/${currentModel}:generateContent?key=${activeKey}`;

            res = await fetch(tryEndpoint, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify(payload),
              signal: abortSignal
          ...
          // لو آخر key وكل حاجة فشلت
          if (ki === apiKeys.length - 1) {
            throw new Error(errJson.error?.message || `Gemini API Error: ${res.status}`);
          }
          // غير كده جرب الـ key الجاي
          console.warn(`[GeminiAgent] Key ${ki + 1} failed (${errCode}), trying next key...`);
        }
      };

      const callGemini = (payload) => callWithKeyRotation(payload);
