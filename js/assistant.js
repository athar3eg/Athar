...
(async function init() {
  try {
    ...
    const modelName = localStorage.getItem('athar_gemini_model') || localStorage.getItem('madar_gemini_model') || 'gemini-2.0-flash';
    const badge = document.getElementById("agentBadge");
    const statusText = document.getElementById("agentStatusText");
    if (badge) {
      if (hasKey) {
        badge.innerHTML = `<span class="w-2 h-2 rounded-full bg-secondary animate-pulse"></span><span>${modelName} ⚡</span>`;
        badge.classList.remove("bg-outline-variant", "text-on-surface-variant");
        badge.classList.add("bg-secondary-container", "text-on-secondary-container");
      } else {
        badge.innerHTML = `<span class="w-2 h-2 rounded-full bg-outline animate-pulse"></span><span>وضع افتراضي</span>`;
        badge.classList.remove("bg-secondary-container", "text-on-secondary-container");
        badge.classList.add("bg-outline-variant", "text-on-surface-variant");
      }
    }
    if (statusText) {
      statusText.textContent = hasKey
        ? `تحكم كامل عبر ${modelName}`
        : "متصل بجدولك وحصصك وموادك الدراسية";
    }
